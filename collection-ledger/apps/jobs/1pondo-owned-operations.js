...
function candidateCodes(fileName, allowRenamed = false) {
  ...
  if (!lower.includes("1pon") && !allowRenamed) return [];
  const result = [];
  ...
  for (const match of stem.matchAll(/([0-9]{6})[_-]([0-9A-Za-z]{3})(?![0-9A-Za-z])/g)) {
    result.push(`${match[1]}_${match[2]}`);
  }
  for (const match of stem.matchAll(/([0-9]{6})[_-]([0-9A-Za-z]{3})(?=[_-](?:1pon|1pondo)(?:[_-]|$))/gi)) {
    result.push(`${match[1]}_${match[2]}`);
  }
...
async function fetchMasterMap(client) {
  const result = await client.query(`select movie_code, relation_key_mmddyy, title, coalesce(actor_name,'') actor_name from cl.${DB_PREFIX}_m005_master`);
...
async function readApprovedPlan(planCsv, mode, inputDir = "", client = null) {
  ...
  for (const row of rows) {
    ...
    if (mode === "master-complement") {
      if (row.status !== "ready" || !/^[0-9]{6}_[0-9A-Za-z]{3}$/.test(row.movie_code)) throw new Error(`Invalid master complement row: ${row.source_path}`);
...
async function buildMissingMasterPlan(client) {
  ...
  for (const drive of DRIVES) {
    ...
    for (const filePath of scan.files) {
      ...
      const explicitCodes = [...new Set(candidates.filter((value) => /^[0-9]{6}_[0-9A-Za-z]{3}$/.test(value)))];
...
async function applyMissingMasters(client, rows) {
  const unsafe = rows.filter((row) => row.status !== "ready" || !/^[0-9]{6}_[0-9A-Za-z]{3}$/.test(row.movie_code));
  if (unsafe.length) throw new Error(`Missing master plan has unsafe rows: ${unsafe.length}`);
  await client.query("begin");
  try {
    let inserted = 0;
    for (const row of rows) {
      const detailUrl = `https://www.1pondo.tv/movies/${row.movie_code}/`;
      const thumbnailUrl = `https://www.1pondo.tv/moviepages/${row.movie_code}/images/str.jpg`;
      const result = await client.query(`insert into cl.${DB_PREFIX}_m005_master
        ...
        values ($1,$2,$3,$4,$5,$6,'1Pondo',$7,$8,'owned_file_added','Added from explicit owned filename after review')
  ...
  try {
    for (const row of rows) {
      ...
      await client.query(`insert into cl.${DB_PREFIX}_owned_file (movie_code,file_path,file_name,file_ext,drive_letter,file_size_bytes,file_mtime,last_seen_at,note)
        values ($1,$2,$3,$4,$5,$6,$7,now(),'Imported by 1pondo-owned-operations')
