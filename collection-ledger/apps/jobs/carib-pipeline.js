...
const MASTER_URL = "https://www.caribbeancom.com/listpages/all{page}.htm";
const HIJAV_URL = "https://hijav.net/category/jav-uncensored/caribbeancom/page/{page}/";
...
const ENABLE_DL_DETAIL_FALLBACK = true;
...
function originalMovieCode(value) {
  const match = String(value || "").match(/([0-9]{6}[-_][0-9A-Za-z]{3})(?![0-9A-Za-z])/);
...
function parseReleaseDate(value) {
  ...
  return { releaseDate: match ? `${match[1]}-${match[2]}-${match[3]}` : null, releaseDateText: text };
}
...
function parseMasterHtml(html, pageUrl, pageNumber) {
  ...
  const seen = new Set();
  $(".entry").each((entryIndex, entry) => {
    const href = String($(entry).find("a[href*='/moviepages/']").first().attr("href") || "");
    const movieCode = normalizeMovieCode(href);
    if (!movieCode || seen.has(movieCode)) return;
    seen.add(movieCode);
    const img = $(entry).find("img[itemprop='thumbnail'], img.media-image, img").first();
    const title = cleanText($(entry).find(".meta-title a").first().text() || img.attr("title") || movieCode);
    const release = parseReleaseDate($(entry).find(".meta-data").first().text());
    const actors = $(entry).find("[itemprop='actor'] [itemprop='name'], [itemprop='actor']").toArray().map((actor) => cleanText($(actor).text())).filter(Boolean);
    const thumbnailUrl = normalizeUrl(img.attr("src"), pageUrl);
    const detailUrl = normalizeUrl(href, pageUrl);
    const originalCode = originalMovieCode(href) || movieCode;
    rows.push({
      movie_code: movieCode,
      original_movie_code: originalCode,
      relation_key_mmddyy: relationKey(movieCode, release.releaseDate),
      release_date: release.releaseDate,
      release_date_text: release.releaseDateText,
      movie_code_suffix: movieCode.replace(/^[0-9]{6}[-_]?/, ""),
      title: title || movieCode,
      actor_name: [...new Set(actors)].join(", "),
      channel_name: "Caribbeancom",
      ...
      raw_payload: {
        href,
        original_movie_code: originalCode,
        item_text: cleanText($(entry).text()),
        image_alt: cleanText(img.attr("alt") || ""),
        sample_video_url: cleanText(img.attr("data-video-src") || ""),
      },
    });
  });
  return rows;
...
async function insertMasterRows(rows, pagesRequested, pageLogs) {
  ...
  try {
    ...
    await client.query(`insert into cl.${DB_PREFIX}_m007_master_collect_runs (run_id,status,pages_requested,pages_processed,rows_collected) values ($1,'running',$2,$3,$4)`, [id, pagesRequested, pageLogs.length, rows.length]);
    for (const pageLog of pageLogs) {
      await client.query(`insert into cl.${DB_PREFIX}_m007_master_page_logs (run_id,page_number,page_url,rows_found) values ($1,$2,$3,$4)`,
        [id, pageLog.page_number, pageLog.page_url, pageLog.rows_found]);
    }
    for (const row of rows) {
      const raw = await client.query(`
        insert into cl.${DB_PREFIX}_m007_master_raw
          (relation_key_mmddyy,release_date,release_date_text,movie_code,original_movie_code,movie_code_suffix,title,actor_name,channel_name,detail_url,thumbnail_url,source_page_url,page_number,row_index_in_page,raw_payload,last_run_id)
        ...
        on conflict (movie_code) do update set release_date=excluded.release_date, release_date_text=excluded.release_date_text, relation_key_mmddyy=excluded.relation_key_mmddyy,
          original_movie_code=excluded.original_movie_code,movie_code_suffix=excluded.movie_code_suffix,title=excluded.title,actor_name=excluded.actor_name,detail_url=excluded.detail_url,
        ...
        returning id`, [row.relation_key_mmddyy, row.release_date, row.release_date_text, row.movie_code, row.original_movie_code, row.movie_code_suffix, row.title, row.actor_name || null, row.channel_name, row.detail_url, row.thumbnail_url, row.source_page_url, row.page_number, row.row_index_in_page, JSON.stringify(row.raw_payload), id]);
      await client.query(`
        insert into cl.${DB_PREFIX}_m007_master
          (movie_code,relation_key_mmddyy,release_date,release_date_text,original_movie_code,movie_code_suffix,title,actor_name,channel_name,detail_url,thumbnail_url,raw_id,last_run_id)
        ...
        on conflict (movie_code) do update set relation_key_mmddyy=excluded.relation_key_mmddyy,release_date=excluded.release_date,release_date_text=excluded.release_date_text,
          original_movie_code=excluded.original_movie_code,movie_code_suffix=excluded.movie_code_suffix,title=excluded.title,actor_name=excluded.actor_name,channel_name=excluded.channel_name,
      ...
      [row.movie_code, row.relation_key_mmddyy, row.release_date, row.release_date_text, row.original_movie_code, row.movie_code_suffix, row.title, row.actor_name || null, row.channel_name, row.detail_url, row.thumbnail_url, raw.rows[0].id, id]);
    }
    await client.query(`update cl.${DB_PREFIX}_m007_master_collect_runs set status='success', finished_at=now() where run_id=$1`, [id]);
...
function validateThumbnailUrl(url) {
  ...
  return parsed.protocol === "https:" &&
    ["tarimages.caribbeancom.com", "www.caribbeancom.com", "caribbeancom.com"].includes(parsed.hostname);
}
...
async function collectThumbnails(args) {
  ...
  try {
    const result = await client.query(`
      ...
      from cl.${DB_PREFIX}_m007_master master
      left join cl.${DB_PREFIX}_m007_thumbnail_assets asset on asset.movie_code = master.movie_code
      where coalesce(master.thumbnail_url,'')<>''
        and coalesce(asset.thumbnail_status, 'pending') <> 'collected'
      order by case coalesce(asset.thumbnail_status, 'pending') when 'pending' then 0 when 'failed' then 1 else 2 end, master.movie_code
    ...
    for (const row of result.rows) {
      try {
        ...
        await client.query(`insert into cl.${DB_PREFIX}_m007_thumbnail_assets (movie_code,thumbnail_url,local_thumbnail_path,local_thumbnail_file_name,thumbnail_status,bytes,last_checked_at,downloaded_at)
        ...
        await client.query(`update cl.${DB_PREFIX}_m007_master set thumbnail_file_path=$2,updated_at=now() where movie_code=$1`, [row.movie_code, outputPath]);
      } catch (error) {
        summary.failed += 1;
        await client.query(`insert into cl.${DB_PREFIX}_m007_thumbnail_assets (movie_code,thumbnail_url,thumbnail_status,last_error,last_checked_at) values ($1,$2,'failed',$3,now())
...
function extractMovieCodeCandidates(...values) {
  ...
  for (const value of values) {
    ...
    for (const match of text.matchAll(/([0-9]{6})[-_]([0-9A-Za-z]{3})(?![0-9A-Za-z])/g)) {
      candidates.push(`${match[1]}-${match[2]}`.toLowerCase());
    }
  }
...
async function fetchMasterResolver() {
  ...
  await client.connect();
  try {
    const result = await client.query(`select movie_code, relation_key_mmddyy from cl.${DB_PREFIX}_m007_master`);
...
function resolveMovieCode(candidates, resolver) {
  ...
  if (allMatches.length === 1) return { movieCode: allMatches[0], matchReason: "exact" };
...
async function collectDl(args) {
  ...
  for (let offset = 0; offset < args.maxPages; offset += 1) {
    ...
    for (const post of posts) {
      ...
      if (ENABLE_DL_DETAIL_FALLBACK && !links.length && detailUrl) {
        if (!isApprovedHijavUrl(detailUrl)) {
          foundSource = "error";
          links = [];
        } else {
        detailFetchCount += 1;
  ...
  if (!hasErrors && args.limit === 0 && pageLogs.length === args.maxPages) {
    ...
    try {
      ...
      pageLogs.push({ run_id: id, page_number: terminalPageNumber, page_url: terminalPageUrl, status: terminalPosts > 0 ? "error" : "terminal", posts_found: terminalPosts, rows_found: 0, list_rg_found: 0, detail_fetch_count: 0, detail_rg_found: 0, error_message: terminalPosts > 0 ? "More pages exist beyond max-pages" : "No expected post elements found on terminal probe" });
    } catch (error) {
      pageLogs.push({ run_id: id, page_number: terminalPageNumber, page_url: terminalPageUrl, status: error.status === 404 ? "terminal" : "error", posts_found: 0, rows_found: 0, list_rg_found: 0, detail_fetch_count: 0, detail_rg_found: 0, error_message: error.status === 404 ? "" : error.message });
    }
  }
...
async function initDb() {
  const client = createPgClient();
  await client.connect();
  try { await client.query(fs.readFileSync(path.resolve(__dirname, "..", "..", "ops", "sql", "130_carib_site.sql"), "utf8")); }
  finally { await client.end(); }
}
...
async function status() {
  ...
  try {
    const result = await client.query(`select
      (select count(*)::integer from cl.${DB_PREFIX}_m007_master) master_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m007_master_raw) raw_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m007_master where release_date is null) master_null_release_date_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m007_thumbnail_assets where thumbnail_status='collected') thumbnail_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m007_thumbnail_assets where thumbnail_status='failed') thumbnail_failed_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m007_master where coalesce(thumbnail_url,'')<>'' and movie_code not in
        (select movie_code from cl.${DB_PREFIX}_m007_thumbnail_assets where thumbnail_status in ('collected','failed'))) thumbnail_pending_count,
    ...
    const failures = await client.query(`select movie_code,thumbnail_url,last_error from cl.${DB_PREFIX}_m007_thumbnail_assets where thumbnail_status='failed' order by movie_code`);
...
async function seedCommonMasters() {
  ...
  try {
    await client.query("begin");
    const siteResult = await client.query("select site_id from cl.site_master where site_code='carib'");
    if (!siteResult.rowCount) throw new Error("site_master row is missing for carib");
    const siteId = siteResult.rows[0].site_id;
    const actorResult = await client.query(`select distinct trim(value) actor_name
      from cl.${DB_PREFIX}_m007_master,
      lateral regexp_split_to_table(coalesce(actor_name,''), '\\s*[,、/]\\s*') value
      where trim(value)<>''`);
    let insertedGroups = 0;
    let insertedNames = 0;
    for (const row of actorResult.rows) {
      const groupCode = `carib:${row.actor_name}`;
      const group = await client.query(`insert into cl.actor_group_master (group_code,representative_actor_name,note)
        values ($1,$2,'Seeded from Caribbeancom master') on conflict (group_code) do update set representative_actor_name=excluded.representative_actor_name,updated_at=now()
      ...
      const name = await client.query(`insert into cl.actor_name_master (actor_name,site_id,actor_group_id,note)
        values ($1,$2,$3,'Seeded from Caribbeancom master') on conflict (site_id,actor_name) do update set actor_group_id=excluded.actor_group_id,updated_at=now()`,
