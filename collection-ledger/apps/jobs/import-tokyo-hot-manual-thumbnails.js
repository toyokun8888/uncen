...
const DEFAULT_INPUT_DIR = "P:\\uncen\\tokyo_hot_thumbnails";
...
function normalizeMovieCode(value) {
  const match = String(value || "").toLowerCase().match(/^n[\s._-]*0*([0-9]{1,5})$/);
  if (!match) return "";
  return `n${String(Number(match[1])).padStart(4, "0")}`;
}
...
async function buildPlan(client, inputDir) {
  const masters = await client.query(`select movie_code,title from cl.${DB_PREFIX}_${MASTER_NO}_master`);
  const masterMap = new Map(masters.rows.map((row) => [row.movie_code, row]));
  const files = listImages(inputDir);
  const counts = new Map();
  for (const file of files) {
    const key = normalizeMovieCode(path.parse(file).name) || path.parse(file).name.toLowerCase();
    counts.set(key, (counts.get(key) || 0) + 1);
  }
  return files.map((file) => {
    const key = normalizeMovieCode(path.parse(file).name);
    const ext = path.extname(file).toLowerCase() === ".jpeg" ? ".jpg" : path.extname(file).toLowerCase();
    const stat = fs.statSync(file);
    let status = "ready";
    if (!key) status = "invalid_unique_key";
    else if ((counts.get(key) || 0) > 1) status = "duplicate_unique_key";
    else if (!stat.size || !isSupportedImage(file)) status = "invalid_image_file";
...
async function readApprovedPlan(client, inputDir, planCsv) {
  ...
  for (const row of rows) {
    ...
    const master = await client.query(`select 1 from cl.${DB_PREFIX}_${MASTER_NO}_master where movie_code=$1`, [row.movie_code]);
...
async function apply(client, rows) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await client.query(`insert into cl.${DB_PREFIX}_${MASTER_NO}_thumbnail_assets (movie_code,local_thumbnail_path,local_thumbnail_file_name,thumbnail_status,bytes,last_checked_at,downloaded_at)
      ...
      await client.query(`update cl.${DB_PREFIX}_${MASTER_NO}_master set thumbnail_file_path=$2,updated_at=now() where movie_code=$1`, [row.movie_code, row.output_path]);
    ...
    try { fs.unlinkSync(row.source_path); } catch (error) { process.stderr.write(`warning: imported thumbnail but could not remove source image: ${row.source_path}: ${error.message}\n`); }
  }
...
async function main() {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({ ok: true, source: SOURCE, step: args.step, input_dir: args.inputDir, output_path: outputPath, result }, null, 2)}\n`);
  } finally {
    await client.end();
  }
}
