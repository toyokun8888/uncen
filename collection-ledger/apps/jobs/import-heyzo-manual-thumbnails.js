...
const DEFAULT_INPUT_DIR = "P:\\uncen\\heyzo_thumbnails";
...
async function buildPlan(client, inputDir) {
  const masters = await client.query(`select movie_code,title from cl.${DB_PREFIX}_m004_master`);
  const masterMap = new Map(masters.rows.map((row) => [row.movie_code, row]));
  const files = listImages(inputDir);
  const counts = new Map();
  for (const file of files) {
    const movieCode = normalizeMovieCode(path.parse(file).name) || path.parse(file).name;
    counts.set(movieCode, (counts.get(movieCode) || 0) + 1);
  }
...
async function readApprovedPlan(client, inputDir, planCsv) {
  ...
  for (const row of rows) {
    ...
    const master = await client.query(`select 1 from cl.${DB_PREFIX}_m004_master where movie_code=$1`, [row.movie_code]);
...
async function apply(client, rows) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await client.query(`insert into cl.${DB_PREFIX}_m004_thumbnail_assets (movie_code,local_thumbnail_path,local_thumbnail_file_name,thumbnail_status,bytes,last_checked_at,downloaded_at)
      ...
      await client.query(`update cl.${DB_PREFIX}_m004_master set thumbnail_file_path=$2,updated_at=now() where movie_code=$1`, [row.movie_code, row.output_path]);
