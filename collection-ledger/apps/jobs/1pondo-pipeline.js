...
const MASTER_URL = "https://www.1pondo.tv/dyn/phpauto/movie_lists/list_newest_{offset}.json";
const HIJAV_URL = "https://hijav.net/category/jav-uncensored/1pondo/page/{page}/";
...
function normalizeMovieCode(value) {
  const match = String(value || "").match(/([0-9]{6})[_-]([0-9A-Za-z]{3})/);
  return match ? `${match[1]}_${match[2]}` : "";
}
function officialThumbnailUrl(movieCode, value, baseUrl) {
  const fallback = `https://www.1pondo.tv/moviepages/${movieCode}/images/str.jpg`;
  const normalized = normalizeUrl(cleanText(value), baseUrl);
  if (!normalized) return fallback;
  try {
    const parsed = new URL(normalized);
    if (parsed.hostname === "moviepages") return fallback;
    return normalized;
  } catch {
    return fallback;
  }
...
async function collectMaster(args) {
  ...
  for (let pageOffset = 0; pageOffset < args.maxPages; pageOffset += 1) {
    const offset = (args.startPage - 1 + pageOffset) * 50;
    const pageUrl = MASTER_URL.replace("{offset}", String(offset));
    const response = await fetch(pageUrl, { headers: { "user-agent": "Mozilla/5.0", "accept-language": "ja,en-US;q=0.9" } });
    if (!response.ok) throw new Error(`Master JSON fetch failed: HTTP ${response.status} ${pageUrl}`);
    const payload = await response.json();
    const pageRows = parseMasterJson(payload, pageUrl, offset);
    for (const row of pageRows) {
      rows.push(row);
      if (args.limit > 0 && rows.length >= args.limit) break;
    }
    pageLogs.push({ page_number: offset, page_url: pageUrl, rows_found: pageRows.length });
    if (args.limit > 0 && rows.length >= args.limit) break;
...
function parseMasterJson(payload, pageUrl, offset) {
  const rows = [];
  const sourceRows = Array.isArray(payload?.Rows) ? payload.Rows : [];
  for (const [index, item] of sourceRows.entries()) {
    const movieCode = normalizeMovieCode(item.MovieID);
    const release = parseReleaseDate(item.Release);
    const actorName = cleanText(item.Actor || item.ActressesJa || item.ActressesEn || "");
    const thumbnailUrl = officialThumbnailUrl(movieCode, item.ThumbHigh || item.ThumbMed || item.MovieThumb || item.ThumbLow || "", pageUrl);
    ...
    rows.push({
      ...
      title: cleanText(item.Title || item.TitleEn || movieCode),
      actor_name: actorName,
      channel_name: "1Pondo",
      detail_url: `https://www.1pondo.tv/movies/${movieCode}/`,
      thumbnail_url: thumbnailUrl || `https://www.1pondo.tv/moviepages/${movieCode}/images/str.jpg`,
      source_page_url: pageUrl,
      page_number: offset,
      row_index_in_page: index + 1,
      raw_payload: {
        ...item,
        source_json_url: pageUrl,
        offset,
        total_rows: payload?.TotalRows,
        split_size: payload?.SplitSize,
      },
    });
  }
  return rows;
...
async function insertMasterRows(rows, pagesRequested, pageLogs) {
  ...
  try {
    ...
    await client.query(`insert into cl.${DB_PREFIX}_m005_master_collect_runs (run_id,status,pages_requested,pages_processed,rows_collected) values ($1,'running',$2,$3,$4)`, [id, pagesRequested, pageLogs.length, rows.length]);
    for (const pageLog of pageLogs) {
      await client.query(`insert into cl.${DB_PREFIX}_m005_master_page_logs (run_id,page_number,page_url,rows_found) values ($1,$2,$3,$4)`,
        [id, pageLog.page_number, pageLog.page_url, pageLog.rows_found]);
    }
    for (const row of rows) {
      const raw = await client.query(`
        insert into cl.${DB_PREFIX}_m005_master_raw
      ...
      await client.query(`
        insert into cl.${DB_PREFIX}_m005_master
    ...
    await client.query(`update cl.${DB_PREFIX}_m005_master_collect_runs set status='success', finished_at=now() where run_id=$1`, [id]);
...
function validateThumbnailUrl(url) {
  ...
  return parsed.protocol === "https:" &&
    ["www.1pondo.tv", "1pondo.tv"].includes(parsed.hostname) &&
...
async function collectThumbnails(args) {
  ...
  await client.connect();
  try {
    const result = await client.query(`select movie_code, thumbnail_url from cl.${DB_PREFIX}_m005_master where coalesce(thumbnail_url,'')<>'' order by movie_code limit $1`, [args.limit > 0 ? args.limit : 100000]);
    ...
    for (const row of result.rows) {
      try {
        ...
        await client.query(`insert into cl.${DB_PREFIX}_m005_thumbnail_assets (movie_code,thumbnail_url,local_thumbnail_path,local_thumbnail_file_name,thumbnail_status,bytes,last_checked_at,downloaded_at)
        ...
        await client.query(`update cl.${DB_PREFIX}_m005_master set thumbnail_file_path=$2,updated_at=now() where movie_code=$1`, [row.movie_code, outputPath]);
      } catch (error) {
        summary.failed += 1;
        await client.query(`insert into cl.${DB_PREFIX}_m005_thumbnail_assets (movie_code,thumbnail_url,thumbnail_status,last_error,last_checked_at) values ($1,$2,'failed',$3,now())
...
function extractMovieCodeCandidates(...values) {
  ...
  for (const value of values) {
    ...
    for (const match of text.matchAll(/([0-9]{6})[_-]([0-9A-Za-z]{3})(?![0-9A-Za-z])/g)) {
      candidates.push(`${match[1]}_${match[2]}`);
    }
  }
...
async function fetchMasterResolver() {
  ...
  await client.connect();
  try {
    const result = await client.query(`select movie_code, relation_key_mmddyy from cl.${DB_PREFIX}_m005_master`);
...
async function initDb() {
  const client = createPgClient();
  await client.connect();
  try { await client.query(fs.readFileSync(path.resolve(__dirname, "..", "..", "ops", "sql", "110_1pondo_site.sql"), "utf8")); }
  finally { await client.end(); }
}
...
async function status() {
  ...
  try {
    const result = await client.query(`select
      (select count(*)::integer from cl.${DB_PREFIX}_m005_master) master_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m005_master_raw) raw_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m005_master where release_date is null) master_null_release_date_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m005_thumbnail_assets where thumbnail_status='collected') thumbnail_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m005_thumbnail_assets where thumbnail_status='failed') thumbnail_failed_count,
      (select count(*)::integer from cl.${DB_PREFIX}_m005_master where coalesce(thumbnail_url,'')<>'' and movie_code not in
        (select movie_code from cl.${DB_PREFIX}_m005_thumbnail_assets where thumbnail_status in ('collected','failed'))) thumbnail_pending_count,
    ...
    const failures = await client.query(`select movie_code,thumbnail_url,last_error from cl.${DB_PREFIX}_m005_thumbnail_assets where thumbnail_status='failed' order by movie_code`);
...
async function seedCommonMasters() {
  ...
  try {
    await client.query("begin");
    const siteResult = await client.query("select site_id from cl.site_master where site_code='1pondo'");
    if (!siteResult.rowCount) throw new Error("site_master row is missing for 1pondo");
    const siteId = siteResult.rows[0].site_id;
    const actorResult = await client.query(`select distinct trim(value) actor_name
      from cl.${DB_PREFIX}_m005_master,
      lateral regexp_split_to_table(coalesce(actor_name,''), '\\s*[,、/]\\s*') value
      where trim(value)<>''`);
    let insertedGroups = 0;
    let insertedNames = 0;
    for (const row of actorResult.rows) {
      const groupCode = `1pondo:${row.actor_name}`;
      const group = await client.query(`insert into cl.actor_group_master (group_code,representative_actor_name,note)
        values ($1,$2,'Seeded from 1Pondo master') on conflict (group_code) do update set representative_actor_name=excluded.representative_actor_name,updated_at=now()
      ...
      const name = await client.query(`insert into cl.actor_name_master (actor_name,site_id,actor_group_id,note)
        values ($1,$2,$3,'Seeded from 1Pondo master') on conflict (site_id,actor_name) do update set actor_group_id=excluded.actor_group_id,updated_at=now()`,
