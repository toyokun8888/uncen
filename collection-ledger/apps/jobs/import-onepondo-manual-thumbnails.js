...
const DEFAULT_INPUT_DIR = "P:\\uncen\\1pondo_thumbnails";
...
async function buildPlan(client, inputDir) {
  const masters = await client.query(`select movie_code,title from cl.${DB_PREFIX}_m005_master`);
  ...
  return files.map((file) => {
    ...
    let status = "ready";
    if (!/^[0-9]{6}_[0-9A-Za-z]{3}$/.test(key)) status = "invalid_unique_key";
...
async function readApprovedPlan(client, inputDir, planCsv) {
  ...
  for (const row of rows) {
    ...
    const master = await client.query(`select 1 from cl.${DB_PREFIX}_m005_master where movie_code=$1`, [row.movie_code]);
...
async function apply(client, rows) {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await client.query(`insert into cl.${DB_PREFIX}_m005_thumbnail_assets (movie_code,local_thumbnail_path,local_thumbnail_file_name,thumbnail_status,bytes,last_checked_at,downloaded_at)
      ...
      await client.query(`update cl.${DB_PREFIX}_m005_master set thumbnail_file_path=$2,updated_at=now() where movie_code=$1`, [row.movie_code, row.output_path]);
