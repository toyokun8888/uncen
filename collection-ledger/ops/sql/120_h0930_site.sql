...
values ('h0930', 'H0930', 'h0930 browser source')
on conflict (site_code) do update set
    site_name = excluded.site_name,
    note = excluded.note,
    updated_at = now();

create table if not exists cl.h0930_m006_master_raw (
...
alter table cl.h0930_m006_master_raw
    add column if not exists release_date_text text;

create table if not exists cl.h0930_m006_master (
    ...
    title text not null,
    actor_name text,
    channel_name text not null default 'H0930',
    detail_url text,
    thumbnail_url text,
    thumbnail_file_path text,
    raw_id bigint references cl.h0930_m006_master_raw(id),
...
alter table cl.h0930_m006_master
    add column if not exists release_date_text text;

create index if not exists h0930_m006_master_relation_key_idx
    on cl.h0930_m006_master (relation_key_mmddyy);

create table if not exists cl.h0930_m006_master_collect_runs (
...
create table if not exists cl.h0930_m006_master_page_logs (
    page_log_id bigserial primary key,
    run_id text not null references cl.h0930_m006_master_collect_runs(run_id) on delete cascade,
    page_number integer not null,
...
create table if not exists cl.h0930_m006_thumbnail_assets (
    movie_code text primary key references cl.h0930_m006_master(movie_code) on delete cascade,
    thumbnail_url text,
...
create table if not exists cl.h0930_owned_file (
    owned_file_id bigserial primary key,
    movie_code text not null references cl.h0930_m006_master(movie_code),
...
create index if not exists h0930_owned_file_movie_code_idx
    on cl.h0930_owned_file (movie_code);

create table if not exists cl.h0930_owned_file_video_metadata (
    owned_file_id bigint primary key references cl.h0930_owned_file(owned_file_id) on delete cascade,
    ...
    constraint h0930_owned_file_video_metadata_resolution_class_chk
        check (resolution_class is null or resolution_class in ('4k', 'hd', 'low')),
    constraint h0930_owned_file_video_metadata_probe_status_chk
        check (probe_status in ('pending', 'ok', 'failed', 'file_missing', 'path_not_allowed', 'unsupported'))
);

create index if not exists h0930_owned_file_video_metadata_movie_code_idx
    on cl.h0930_owned_file_video_metadata (movie_code);

create index if not exists h0930_owned_file_video_metadata_resolution_class_idx
    on cl.h0930_owned_file_video_metadata (resolution_class);

create index if not exists h0930_owned_file_video_metadata_probe_status_idx
    on cl.h0930_owned_file_video_metadata (probe_status);

create table if not exists cl.h0930_dl_reference (
...
create table if not exists cl.h0930_dl_reference_runs (
    run_id text primary key,
    status text not null,
...
alter table cl.h0930_dl_reference_runs
    add column if not exists error_count integer not null default 0;

create table if not exists cl.h0930_dl_reference_page_logs (
    page_log_id bigserial primary key,
    run_id text not null references cl.h0930_dl_reference_runs(run_id) on delete cascade,
    page_number integer not null,
    page_url text not null,
    status text not null,
    posts_found integer not null default 0,
...
create unique index if not exists h0930_dl_reference_identity_uidx
    on cl.h0930_dl_reference (
        coalesce(movie_code, ''),
        coalesce(detail_url, ''),
        coalesce(rapidgator_url, '')
    );

create or replace view cl.h0930_v_thumbnail_assets as
select
    master.movie_code,
...
from cl.h0930_m006_master master
left join cl.h0930_m006_thumbnail_assets asset on asset.movie_code = master.movie_code;

create or replace view cl.h0930_v_library_items as
select
    owned.owned_file_id,
    'h0930'::text as site_code,
    'H0930'::text as site_name,
    master.movie_code,
    master.release_date,
    ...
    owned.created_at,
    owned.updated_at
from cl.h0930_owned_file owned
join cl.h0930_m006_master master on master.movie_code = owned.movie_code
left join cl.h0930_owned_file_video_metadata video_metadata on video_metadata.owned_file_id = owned.owned_file_id
left join cl.h0930_v_thumbnail_assets thumb on thumb.movie_code = master.movie_code
left join lateral (
    select
...
create or replace view cl.h0930_v_completion_items as
...
grant select, insert, update, delete on
    cl.h0930_m006_master_raw,
    cl.h0930_m006_master,
    cl.h0930_m006_master_collect_runs,
    cl.h0930_m006_master_page_logs,
    cl.h0930_m006_thumbnail_assets,
    cl.h0930_owned_file,
    cl.h0930_owned_file_video_metadata,
    cl.h0930_dl_reference,
    cl.h0930_dl_reference_runs,
    cl.h0930_dl_reference_page_logs
to current_user;
grant select on
    cl.h0930_v_thumbnail_assets,
    cl.h0930_v_library_items,
    cl.h0930_v_completion_items
to current_user;
grant usage, select on sequence
    cl.h0930_m006_master_raw_id_seq,
    cl.h0930_m006_master_page_logs_page_log_id_seq,
    cl.h0930_owned_file_owned_file_id_seq,
    cl.h0930_dl_reference_dl_reference_id_seq,
    cl.h0930_dl_reference_page_logs_page_log_id_seq
to current_user;
